...
def model_spec(logspec, f_mod, dn=0.1, df=0.01):
    ...
    n_best = 2
    fc_best = -2
    for n in np.arange(1.0, 3.5, dn, dtype=np.float64):
...
ind_select = np.where(np.logical_and(nbin > 5, depth > 00.0))[0]
...
mw_group[0] = mw[np.logical_and(mw <= 9, mw > 7.6)]
event_group[0] = evid[np.logical_and(mw <= 9, mw > 7.6)]
fc_deno_group[0] = corner_freq_deno[np.logical_and(mw <= 9, mw > 7.6)]
...
f_mod = np.arange(np.log10(0.02), np.log10(1.0), 0.01)
...
for j in np.arange(0, 6):
    ...
    N_events = int(len(mw_group[j]) * 0.6)
    ...
    ax[0].loglog(np.power(10, f_new), bootstrap_ave_noisy, ls='-', color='k', lw=3)
    ax[0].loglog(np.power(10, f_mod), bootstrap_ave_noisy[0] / (1.0 + 10 ** ((f_mod - fc_noisy) * n_noisy)), ls='--', dashes=(3,2), color='b', lw=4)
    ax[0].loglog(np.power(10, fc_noisy), bootstrap_ave_noisy[0] / 2.0, 'o', markersize=15, mec='k', mfc='orange')

    if j==0:
        ax[1].loglog(np.power(10, f_new), bootstrap_ave_deno, ls='-', color='k', lw=3, label='Bootstraping mean')
        ax[1].loglog(np.power(10, f_mod), bootstrap_ave_deno[0] / (1.0 + 10 ** ((f_mod - fc_deno) * n_deno)), ls='--', color='b', lw=4, label='Model spectra')
        ax[1].loglog(np.power(10, fc_deno), bootstrap_ave_deno[0] / 2.0, 'o', markersize=15, mec='k', mfc='orange', label='Corner frequency')
    else:
        ax[1].loglog(np.power(10, f_new), bootstrap_ave_deno, ls='-', color='k', lw=3)
        ax[1].loglog(np.power(10, f_mod), bootstrap_ave_deno[0] / (1.0 + 10 ** ((f_mod - fc_deno) * n_deno)), ls='--', color='b', lw=4)
        ax[1].loglog(np.power(10, fc_deno), bootstrap_ave_deno[0] / 2.0, 'o', markersize=15, mec='k', mfc='orange')
    ...
    for i in range(2):
        ax[i].text(2.2e-2, 1.2 * bootstrap_ave_deno[0], fr'$M_W${7.6 - j*0.4: .1f}-{8.0 - j*0.4: .1f}', fontdict=font)
        ax[i].set_xlabel('frequency (Hz)', fontsize=24)
        ax[i].set_ylabel('amplitude spectrum', fontsize=24)

# %% Fitting lines
lg_mg = np.arange(17.5, 21.5, 0.5, dtype=np.float64)
lg_f = 4.02 - lg_mg / 4
# ax[0].plot(10 ** lg_f, 10 ** lg_mg, '-g', linewidth=5, alpha=1, label=r'$M_0^\frac{1}{4}$~$f_c^{-1}$')
ax[1].plot(10 ** lg_f, 10 ** lg_mg, '-g', linewidth=4, alpha=1, label=r'$M_0^\frac{1}{4}$~$f_c^{-1}$')
# lg_f = 6.00 - lg_mg / 3
# # ax[0].plot(10 ** lg_f, 10 ** lg_mg, '-', color='limegreen', lw=5, label=r'$M_0^\frac{1}{3}$~$f_c^{-1}$')
# ax[1].plot(10 ** lg_f, 10 ** lg_mg, '-', color='limegreen', lw=5, label=r'$M_0^\frac{1}{3}$~$f_c^{-1}$')
# # ax[0].legend(loc='upper right')
ax[1].legend(loc='upper right')

for i in range(2):
    ax[i].grid(which='major', color='#DDDDDD', linewidth=1)
    ax[i].grid(which='minor', color='#EEEEEE', linestyle='--', linewidth=1)
    ax[i].minorticks_on()
    ax[i].set_ylim(1e17, 5e21)
    ax[i].set_xlim(2e-2, 4e0)

plt.savefig(fig_dir + '/stacked_spec_SNR2.pdf')
