...
def main():
    ...
    parser.add_argument('-r', '--minsnr', default=25, type=int, help='lower-bound signal-noise ratio')
    parser.add_argument('-n', '--threads', default=24, type=int, help='number of processes')
    args = parser.parse_args()
    # %% Directories of raw and reformatted data
    data_dir = '/data/whd01/qibin_data/raw_data_for_DenoTe/M6.0plus/2000-2021-include-noFMS/'
    ...
    for i in range(len(cat)):
        ...
        if i == 500 or i == 1000 or i == 1500 or i == (len(cat) - 1):
    ...
    with h5py.File(save_dir + 'chunk1000.hdf5', 'r') as f:
        q2 = f['quake'][:]
        n2 = f['noise'][:]
    with h5py.File(save_dir + 'chunk1500.hdf5', 'r') as f:
        q3 = f['quake'][:]
        n3 = f['noise'][:]
...
#########################################################
    # with h5py.File(save_dir + 'Ssnr20_lp4_2000_2021_include_noFMS.hdf5', 'r') as f:
    #     q0 = f['quake'][:]
    #     n0 = f['noise'][:]
...
#########################################################
    ...
    q1 = np.append(q1, q4, axis=0)
    n1 = np.append(n1, n4, axis=0)

    with h5py.File(save_dir + phase + 'snr' + str(minsnr) + '_lp4_' + '2000-2021-include-noFMS.hdf5', 'w') as f:
        f.create_dataset("quake", data=q1)
        f.create_dataset("noise", data=n1)

    meta.to_csv(save_dir + phase + 'snr' + str(minsnr) + '_lp4_' + '2000-2021-include-noFMS.csv', sep=',', index=False)
...
def one_quake_noise(ev, directory=None, npts=50000, noise_saved_pts=10000, phase='P', minsnr=25):
    ...
    for sta in glob.glob(directory + evnm + 'stas/*xml'):
        ...
        if len(st) >= 3 and len(st[0].data) >= npts and len(st[1].data) >= npts and len(st[2].data) >= npts:
            ...
            noise_amp = np.std(np.array(st[cmp].data)[int(noise_pts - 750): int(noise_pts - 100)])
            quake_amp = np.std(np.array(st[cmp].data)[int(noise_pts): int(noise_pts + 750)])
