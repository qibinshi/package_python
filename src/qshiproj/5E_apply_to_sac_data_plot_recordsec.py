...
from denoiser_util import shift_pad_fix_time, dura_cc
from denoiser_util import shift_pad_stretch_time
from denoiser_util import fit_spec, flux_int
from denoiser_util import directivity3d_free
...
def main():
    ...
    mag = 6.2
    r_energy = 0.90
    sac_dir = '/Users/qibin/Projects/Petrolia_2021/ptr_raw/0_tel_wilber3_pendingprocess/rot_dir'
    ...
    for sta in glob.glob(sac_dir + '/*.z'):
        ...
        if len(st) >= 3:
            tp = UTCDateTime(st[2].stats.starttime - st[2].stats.sac.b + st[2].stats.sac.t0)

            st.trim(tp - preP, tp + time_after)
            if len(st) >= 3 and len(st[0].data) >= npts and len(st[1].data) >= npts and len(st[2].data) >= npts:
                noise_amp = np.std(np.array(st[cmp].data)[int((preP-30.0)/dt):int((preP-5.0)/dt)])
                pwave_amp = np.std(np.array(st[cmp].data)[int((preP-5.0)/dt):int((preP+30.0)/dt)])
            ...
            arrivals = mod.get_travel_times(source_depth_in_km=depth, distance_in_degree=distance, phase_list=['P'])
    ...
    partial_func = denoise_cc_stack(evid, meta_all=meta, input_raw=X_train, model=model, fig_dir=sac_dir, dt=dt, npts=npts, npts_trim=npts_trim, normalized_stack=True, min_snr=snr, cmp=cmp, r_energy=r_energy)
...
def denoise_cc_stack(evid, meta_all=None, input_raw=None, model=None, normalized_stack=True, fix_win=True,
                     npts=None, npts_trim=None, fig_dir=None, dt=0.1, min_snr=5, cmp=2, tstar=0.3, r_energy=0.9):
    ####################################
    ...
    if num_tr >= 1:
        ...
        startpt = int((npts - npts_trim) / 2+100)
        endpt = int((npts + npts_trim) / 2+100)
        ...
        for k in range(2):
            ...
            pos[k] = timex[stack_ener > r_energy * (max_energy - min_energy) + min_energy]
        ...
        for k in range(2):
            ...
            pos = timex[stacked_ener > r_energy * (max_energy - min_energy) + min_energy]
        ...
        # %% Save as a figure
        plt.savefig(fig_dir + '/quake_' + str(evid) + '_record_section_SH.pdf')
